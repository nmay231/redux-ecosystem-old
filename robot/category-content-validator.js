...
const categoryContentErrorFile = join(__dirname, '../category-content-error.md')
const URLRegex = /^https?:\/\/[^\s]([\\][^\s])+$/

const fetchJSON = async (uri) =>
    fetch(uri, { headers: { Authorization: 'token ' + githubToken } }).then((r) => r.json())

// This is hardcoded for now because it would need to be fetched in an async function
const reduxLinksURL =
    'https://api.github.com/repos/markerikson/redux-ecosystem-links/git/trees/804e89f58ec3d828bdb3b1f3d977c41d803de428'
...
function checkCategory(fileName, markdownText) {
    ...
    for (let index = 0; index < childrens.length; ++index) {
        ...
        if (index === 0) {
            if (children.type !== 'heading' || children.depth !== 3) {
                errors.push(
                    'Saw: ' + remark.stringify(children) + ' —— Expected: H3 of category title.',
                )
            }
            continue
        }
    ...
    */
        ...
        if (children.type !== 'heading' || children.depth !== 4) {
            errors.push(
                'Saw: ' + remark.stringify(children) + ' —— Expected: H4 of subcategory title.',
            )
            continue
        }
        ...
        if (index + 1 < childrens.length) {
            ...
            if (mustBeList.type !== 'list' || mustBeList.children.length === 0) {
                errors.push(
                    'Saw: ' + remark.stringify(mustBeList) + ' —— Expected: List of repository.',
                )
                continue
            }
            ...
            mustBeList.children.forEach((mustBeListItem) => {
                if (mustBeListItem.type !== 'listItem' || mustBeListItem.children.length === 0) {
                    errors.push(
                        'Saw: ' +
                            remark.stringify(mustBeListItem) +
                            ' —— Expected: Repository item.',
                    )
                    return
                }
                ...
                if (mustBeParagraph.type !== 'paragraph') {
                    errors.push(
                        'Saw: ' +
                            remark.stringify(mustBeParagraph) +
                            ' —— Expected: Repository item.',
                    )
                    return
                }
                ...
                if (mustBeParagraph.children.length < 1) {
                    ...
                    if (mustBeStrong.type !== 'strong') {
                        errors.push(
                            'Saw: ' +
                                remark.stringify(mustBeStrong) +
                                ' —— Expected: <strong> in repository title.',
                        )
                    } else {
                        if (
                            mustBeStrong.children.length === 0 ||
                            mustBeStrong.children[0].type !== 'text' ||
                            mustBeStrong.children[0].value === ''
                        ) {
                            errors.push(
                                'Saw: ' +
                                    remark.stringify(mustBeStrong.children) +
                                    ' —— Expected: Repository title.',
                            )
                        }
                    }
                ...
                if (mustBeParagraph.children.length < 2) {
                    errors.push(
                        'Saw: ' +
                            remark.stringify(mustBeParagraph) +
                            ' —— Expected: <br /> after repository title.',
                    )
                    return
                } else {
                ...
                if (mustBeParagraph.children.length < 3) {
                    ...
                    if (mustBeGitHubRepoLink.type !== 'link') {
                        errors.push(
                            'Saw: ' +
                                remark.stringify(mustBeGitHubRepoLink) +
                                ' —— Expected: <a> in repository link.',
                        )
                    } else {
                        if (!URLRegex.test(mustBeGitHubRepoLink.url)) {
                            errors.push(
                                'Saw: ' +
                                    remark.stringify(mustBeGitHubRepoLink) +
                                    ' —— Expected: Valid GitHub repository link.',
                            )
                        }
                    }
                }
                ...
                if (mustBeParagraph.children.length < 4) {
                    errors.push(
                        'Saw: ' +
                            remark.stringify(mustBeParagraph) +
                            ' —— Expected: <br /> after repository link.',
                    )
                    return
                } else {
...
;(async function() {
    ...
    const saveFileSpinner = new ora('Save file to ' + categoryContentErrorFile).start()
